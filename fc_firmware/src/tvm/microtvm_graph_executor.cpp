...
std::function<void()> CreateTVMOp(const DSOModule& module, const TVMOpParam& param,
  ...
  // Create tvm args structure
  TVMArgs tvm_args = {};
  tvm_args.values = arg_ptr->arg_values.data();
  tvm_args.tcodes = arg_ptr->arg_tcodes.data();
  tvm_args.values_count = (uint32_t)arg_ptr->arg_values.size();

  auto fexec = [tvm_args, pf]() {
    assert(pf);
    int dummy_ret;
    (pf)(&tvm_args, &dummy_ret);
  };
  return fexec;
}
